...
int binarySearch(std::vector<std::string> strings, std::string x) {
    ...
    while (left <= right) {
        ...
        if (strcmp(s1.c_str(), s2.c_str()) == 0) return mid;
        if (x > strings[mid]) {
            left = mid + 1;
        } else {
            right = mid - 1;
...
Grid::Grid(uint8_t maxGuesses, wordlistID wordlistid, std::string argv0)
  : maxGuesses(maxGuesses), tiles() {
    switch (wordlistid) {
        case CLASSIC: wordlist = wordlist_commonWords; break;
        case FULL: wordlist = wordlist_allWords; break;
        case INSANE:
...
bool Grid::isValidWord(std::string enteredWord) {
    ...
    if (binarySearch(wordlist, word) != -1) return true;
    if (binarySearch(wordlist_commonWords, word) != -1) return true;
    if (binarySearch(wordlist_allWords, word) != -1) return true;
...
std::ostream& operator<<(std::ostream& out, Grid& grid) {
    ...
    for (uint8_t lineIndex = 0; lineIndex < tileSize * grid.maxGuesses; lineIndex++) {
        for (uint8_t charIndex = 0; charIndex < tileSize * grid.wordlength; charIndex++) {
            const Tile t =
              grid.tiles[std::floor(charIndex / tileSize) +
                         grid.wordlength * (std::floor(lineIndex / tileSize))];
            char c = t.let;

            if (c == ' ') { c = '~'; }
            switch (t.type) {
                case TYPE_GREY: printStr += "\033[0m"; break;
                case TYPE_NONE: printStr += "\x1B[38;5;8m"; break;
                case TYPE_GREEN: printStr += "\033[1;32m"; break;
                case TYPE_YELLOW: printStr += "\033[1;33m"; break;
            };
            printStr += c;
            printStr += "\033[0m";
            if (charIndex % tileSize == (tileSize + 1) % 2) { printStr += ' '; }
        }
        printStr += "\n ";
    }
    ...
    for (int i = 97; i <= 123; i++) {
        if ((i - 96) % 14 == 0) letterStates += '\n';
        letterStates += ' ';
        const auto state = grid.charStates[i - 97].state;
        switch (state) {
            case TYPE_GREY: letterStates += "\x1B[38;5;8m"; break;
            case TYPE_NONE: letterStates += "\033[0m"; break;
            case TYPE_GREEN: letterStates += "\033[1;32m"; break;
            case TYPE_YELLOW: letterStates += "\033[1;33m"; break;
        }
        if (i == 123) {
            letterStates += "_\n";
        } else {
